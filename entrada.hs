...
main = do
  ...
  content <- readFile "pastor_copia.txt"--"pastor_copia.txt"
...
ferJugades part@(Partida t c) [x,y] = ((Partida (fesJugada t (Jugada (Peca c (((\l->read[l]) (y!!0))) ((toInteger(digitToInt (y!!2))),(taulerCharToInteger (y!!1)))) (((toInteger(digitToInt (y!!4))),(taulerCharToInteger (y!!3)))))) c),[])
ferJugades part@(Partida t c) (x:y:z:xs) 
  | isNumber (z!!0) = error "ferJugades: el fitxer d'entrada no és correcte"
  | otherwise = ((Partida (fesJugada (jugAnterior part y) (Jugada (Peca (contrari c) (((\l->read[l]) (z!!0))) ((toInteger(digitToInt (z!!2)),(taulerCharToInteger (z!!1))))) (((toInteger(digitToInt (z!!4)),(taulerCharToInteger (z!!3))))))) c),xs)
 where
...
{- game :: Partida -> [String] -> IO ()
game part@(Partida t c) ls = do
    let partidaI = ferJugades part ls
    mostrarTauler (fst partidaI)
    prepSeguent (fst partidaI) (snd partidaI)


prepSeguent :: Partida -> [String] -> IO ()
prepSeguent part@(Partida t c) ls = do -}


{- segJug :: [String] -> [String]
segJug [] = []
segJug [x,y] = []
segJug (x:y:z:xs)
  | isNumber (z!!0) = (z:xs)
